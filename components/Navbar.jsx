...
function Navbar() {
  ...
  return (
    ...
    <header className='z-[999] relative text-black'>
      <motion.div
        className='fixed  top-0 left-1/2 h-[5.9rem] w-full rounded-none border border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[30rem] sm:rounded-full dark:bg-opacity-75 dark:bg-gray-950 dark:border-black/40'
        ...
        <ul className='flex flex-wrap justify-center items-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5'>
            {links.map(link=>(
                <motion.li
                    ...
                    <Link 
                        className={clsx('w-full flex items-center justify-center px-3 py-3 hover:text-gray-950 transition text-base dark:text-gray-500 dark:hover:text-gray-300',
                        {
